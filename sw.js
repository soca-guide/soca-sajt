/* Service Worker - cache version; bump on deploy for instant updates */
const NEW_VERSION = '0803-FINAL';
const CACHE_NAME = 'app-cache-' + NEW_VERSION;
var BASE = self.location.pathname.replace(/\/[^/]*$/, '') || '/';
if (BASE !== '/') BASE += '/';
var STATIC_ASSETS = [
  BASE,
  BASE + 'index.html',
  BASE + 'assets/app.css?v=0803-FINAL',
  BASE + 'assets/config.js?v=0803-FINAL',
  BASE + 'assets/i18n.js?v=0803-FINAL',
  BASE + 'assets/utils.js?v=0803-FINAL',
  BASE + 'assets/content-loader.js?v=0803-FINAL',
  BASE + 'assets/supabase-client.js?v=0803-FINAL',
  BASE + 'assets/tenant-loader.js?v=0803-FINAL',
  BASE + 'assets/app.js?v=0803-FINAL',
  BASE + 'assets/modals.css?v=0803-FINAL',
  BASE + 'assets/modals.js?v=0803-FINAL',
  BASE + 'assets/analytics.js?v=0803-FINAL',
  BASE + 'weather-animations.html?v=0803-FINAL'
];

self.addEventListener('install', function (e) {
  self.skipWaiting();
  e.waitUntil(
    caches.open(CACHE_NAME).then(function (cache) {
      return cache.addAll(STATIC_ASSETS).catch(function () {});
    })
  );
});

self.addEventListener('activate', function (e) {
  e.waitUntil(
    caches.keys().then(function (keys) {
      return Promise.all(
        keys.map(function (key) {
          if (key !== CACHE_NAME) return caches.delete(key);
        })
      );
    }).then(function () { return self.clients.claim(); })
  );
});

self.addEventListener('fetch', function (e) {
  // Guard: skip non-GET and non-http(s) requests (e.g. chrome-extension://)
  if (e.request.method !== 'GET') return;
  try { var _p = new URL(e.request.url).protocol; if (_p !== 'http:' && _p !== 'https:') return; }
  catch (err) { return; }

  var url = e.request.url;
  var isNav = e.request.mode === 'navigate';
  var isHtml = url.indexOf('.html') !== -1 || (url.endsWith('/') || url.endsWith('/index.html'));
  if (isNav || (isHtml && url.startsWith(self.location.origin))) {
    e.respondWith(
      fetch(e.request).then(function (r) { return r; }).catch(function () {
        return caches.match(e.request).then(function (c) { return c || caches.match(BASE + 'index.html'); });
      })
    );
    return;
  }
  if (/\.(css|js|woff2?|ttf|ico|png|jpg|jpeg|gif|webp|svg)(\?|$)/i.test(url)) {
    e.respondWith(
      caches.match(e.request).then(function (c) {
        return c || fetch(e.request).then(function (r) {
          var clone = r.clone();
          if (r.status === 200) caches.open(CACHE_NAME).then(function (cache) { cache.put(e.request, clone); });
          return r;
        });
      })
    );
  }
});
